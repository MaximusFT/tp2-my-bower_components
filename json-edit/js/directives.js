...
.directive('json', ["$compile", function($compile) {
  return {
    ...
    link: function(scope, element, attributes) {
        ...
        if (scope.collapsed) {
            scope.chevron = "fa fa-caret-right";
        } else {
            scope.chevron = "fa fa-caret-down";
        }
        ...
        scope.addItem = function(obj) {
            if (getType(obj) == "Object") {
                ...
                /*} else if (scope.keyName.indexOf("$") == 0){
                    ...
                    alert("The name may not contain . (the dot)");
                } else {
        ...
        '<div ng-switch on="showAddKey" class="block" >'
            ...
            + '<span ng-switch-default>'
                // plus button
                + '<button class="addObjectItemBtn" ng-click="$parent.showAddKey = true"><i class="fa fa-plus"></i></button>'
            + '</span>'
        + '</div>';
    
        // start template
        if (scope.type == "object"){
            var template = '<i ng-click="toggleCollapse()" class="fa mr" ng-class="chevron"></i>'
            + '<span class="jsonItemDesc">'+objectName+'</span>'
            + '<div class="jsonContents" ng-hide="collapsed">'
                // repeat
                + '<span class="block" ng-repeat="(key, val) in child">'
                    // object key
                    + '<span class="jsonObjectKey">'
                        ...
                        // delete button
                        + '<i class="deleteKeyBtn fa fa-trash-o" ng-click="deleteKey(child, key)"></i>'
                    + '</span>'
                    // object value
            ...
            var template = '<i ng-click="toggleCollapse()" class="fa"'
            + 'ng-class="chevron"></i>'
            ...
            + '<div class="jsonContents" ng-hide="collapsed">'
                + '<ol class="arrayOl" ui-sortable="sortableOptions" ng-model="child">'
                    ...
                    + '<li class="arrayItem" ng-repeat="val in child track by $index">'
                        // delete button
                        + '<i class="deleteKeyBtn fa fa-trash-o" ng-click="deleteKey(child, $index)"></i>'
                        + '<i class="moveArrayItemBtn fa fa-align-justify"></i>'
